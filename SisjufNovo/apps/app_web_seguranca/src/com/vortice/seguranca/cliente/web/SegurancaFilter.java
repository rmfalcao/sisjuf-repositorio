...
public class SegurancaFilter implements Filter {
				...
				if (!ext.equals("css") && !ext.equals("gif") && !ext.equals("jpg") && !ext.equals("png") && !ext.equals(".js")){
					if (!login.equals("login.jsf") && !login.equals("login.jsp")){
						if (!mudarSenha.equals("mudarSenha.jsp") && !mudarSenha.equals("mudarSenha.jsf")){
							if (servletPath.indexOf("applet") < 0){//S� filtra se o diretorio passado nao tiver applet
								if(desenv.equals("0")){
									UsuarioVO usuario = (UsuarioVO)sessao.getAttribute("usuario");
									
									if (usuario == null){//TESTO SE O USU�RIO N�O EXISTE NA SESS�O
						                LOG.debug("Usuario n�o existe na sess�o.");
	...
	//									System.out.println("delegate.getPerfilRN() " + delegate.getPerfilRN());
						                ...
						                }else{//SE A ACAO QUE VAI EXECUTAR N�O ESTA NUMA VARIAVEL SETA O LINK COMO A A��O
								...
								} else if (desenv.equals("2")){//O proprio filtro vai salvar o link caso o mesmo n�o exista na base.
	...
	private void proibeContiuacao(ServletRequest request, ServletResponse response) throws ServletException, IOException{
        ...
        script.append("  alert('Voc� n�o tem permiss�o para executar esta fun��o.'); \n");
        script.append("</script> ");
        request.setAttribute("msgP", script.toString());
        LOG.debug("alert('Voc� n�o tem permiss�o para executar esta fun��o.'); ");
