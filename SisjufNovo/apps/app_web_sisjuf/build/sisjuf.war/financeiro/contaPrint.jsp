...
<body>
	...
	<table class="tabela_impressao" cellpadding="3" cellspacing="1">
		...
		<tbody>
			<c:forEach items="${requestScope.contas}" var="item">
			
				<tr>
					<td><c:out value="${item.nome}"/></td>
					<td><c:out value="${item.bancoVO.nome}"/>&nbsp;</td>
					<td><c:out value="${item.numAgencia}"/>&nbsp;</td>
					<td><c:out value="${item.numConta}"/>&nbsp;</td>
					<td><c:out value="${item.nomTitular}"/>&nbsp;</td>
				</tr>
