...
/**
 * Servlet da tela de impressão de origens de lançamento.
 * @author Paulo Prado
 */
public class OrigemLancamentoImpressaoAC extends FrameworkServlet {
	...
	protected void doService(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		request.setAttribute("origensLancamento", financeiroDelegate.findAllOrigemLancamento());
		
		UsuarioVO usuarioVO = (UsuarioVO)  request.getSession().getAttribute("usuario");
		
		request.setAttribute("nomeUsuario", usuarioVO.getNome());
		
		request.getRequestDispatcher("/financeiro/origemLancamentoPrint.jsp").forward(request, response);
	}
