...
/**
 * Servlet da tela de impressão de tipos de lançamento.
 * @author Paulo Prado
 */
public class TipoLancamentoImpressaoAC extends FrameworkServlet {
	...
	protected void doService(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		request.setAttribute("tiposLancamento", financeiroDelegate.findAllTipoLancamento());

		UsuarioVO usuarioVO = (UsuarioVO)  request.getSession().getAttribute("usuario");
		
		request.setAttribute("nomeUsuario", usuarioVO.getNome());
		
		request.getRequestDispatcher("/financeiro/tipoLancamentoPrint.jsp").forward(request, response);
	}
