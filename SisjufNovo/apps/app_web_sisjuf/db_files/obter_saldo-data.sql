-- Function: obter_saldo(int2, date)

-- DROP FUNCTION obter_saldo(int2, date);

CREATE OR REPLACE FUNCTION obter_saldo(int2, date)
  RETURNS "numeric" AS
$BODY$
DECLARE
...
BEGIN
	...
	IF ultima_data IS NULL THEN
		...
		SELECT INTO saldo SUM(bl.val_baixa_lancamento) FROM baixa_lancamento bl NATURAL INNER JOIN lancamento l	
			WHERE l.seq_conta = id_conta AND bl.dat_bixa_lancamento < data_base;
...
ALTER FUNCTION obter_saldo(int2, date) OWNER TO admin_asserjuf;
